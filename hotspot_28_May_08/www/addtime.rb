...
if cgi.has_key?('confirm') or cgi.has_key?('cancel')
  ...
  if cgi.has_key?('confirm')
    ...
    if cgi.has_key?('identifier') and cgi['identifier'].to_s != ''
      # note dup is needed!!!
      record.token = cgi['identifier'].to_s.dup
    end

    if record.state == HotState::PERMANENT
    then
      server.update_record(record)
    elsif cgi['days'].to_i >= 0 and cgi['days'].to_i <= 365 and
