...
class HotServer
   ...
   def update_record_time(mac, days, hours, mins, secs)
     ...
     #update logs
     #17 Sep 07 - zol dont update the logging file
   ...
   def save_data()
      #synchronize do
      3.times { |i|
         File.open(DATAFILE + i.to_s, "w") do |f|
            @log.info 'Writing config ' + i.to_s
            Marshal.dump(@data, f)            
         end
      }
      @log.info 'Done writing configs'
      #end
   end
   
   def load_data()
      begin
         File.open(DATAFILE + '0') do |f|
         ...
         begin
           loop do
              ...
              timed_out.each { |r| 
                 ...
                 elsif r.state != HotState::PERMANENT
                   changed = true
                   @data.records.delete(r)
